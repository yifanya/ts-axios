import { parseHeader, processHeaders, plattenHeaders } from '../../src/helpers/headers'
...
describe('helpers:headers', () => {
  ...
  describe('flattenHeaders', () => {
    test('should flatten the headers and include common headers', () => {
      ...
      expect(plattenHeaders(headers, 'get')).toEqual({
        Accept: 'application/json',
    ...
    test('should flatten the headers without common headers', () => {
      ...
      expect(plattenHeaders(headers, 'patch')).toEqual({
        Accept: 'application/json'
      })
    })

    test('should do nothing if headers is undefined or null', () => {
      expect(plattenHeaders(undefined, 'get')).toBeUndefined()
      expect(plattenHeaders(null, 'post')).toBeNull()
    })
  })
})
